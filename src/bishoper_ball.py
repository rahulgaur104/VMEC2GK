...
def check_ball(shat_n, dPdpsi_n):
        ...
        gds2_n       =  (dpsidrho)**2*(1/R_ex**2 + (dqdr_n*theta_st_com_ex)**2 + (qfac)**2*(dtdr_st_n**2 + \
                        (dt_st_l_ex/dl_ex)**2)+ 2*qfac*dqdr_n*theta_st_com_ex*dtdr_st_n)*1/(a_N*B_N)**2
...
def gamma_ball(shat_n, dPdpsi_n):
        ...
        c = -1*dPdpsi_n*1/B_N**2*dpsidrho*cvdrift_ball*1/(np.abs(gradpar_ball)*B_ball)
        ...
        theta_ball_u_half = np.concatenate([np.array([0.]), (theta_ball_u[len2+1:] + theta_ball_u[len2:-1])/2])  
        ...
        for i in range(len2):
            if i == 0:
                A[i, i:i+2] = np.array([-2*g_u_half[i+1]/f_u1[i]*1/h**2+c_u1[i]/f_u1[i], 2*g_u_half[i+1]/f_u1[i]*1/h**2])   
            elif i == len2-1:
                A[i, i-1:i+1] = np.array([g_u_half[i]/f_u1[i]*1/h**2, -(g_u_half[i]+g_u_half[i+1])/f_u1[i]*1/h**2 + c_u1[i]/f_u1[i]])   
            else:
                A[i, i-1:i+2] = np.array([g_u_half[i]/f_u1[i]*1/h**2, -(g_u_half[i]+g_u_half[i+1])/f_u1[i]*1/h**2 + c_u1[i]/f_u1[i], g_u_half[i+1]/f_u1[i]*1/h**2])   
...
results2 = np.array([[pool.apply_async(gamma_ball, args=(shat_grid[i], dp_dpsi_grid[j])) for i in range(len1)] for j in range(len2)])
for i in range(len2):
...
cs = plt.contour(X.T, Y.T, ball_scan_arr1, levels=[0.])  
