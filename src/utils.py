...
def extract_essence(arr, extract_len, mode=0):
    ...
    # Depending on the value of mode the returned array is reversed(mode=0) or not reversed
...
def reflect_n_append(arr, ch):
	"""
	The purpose of this function is to extend the span of an array from [0, np.pi] to [-np.pi,np.pi). ch can either be 'e'(even) or 'o'(odd) depending upon the parity of the input array.
	"""
	rows = 1
...
def find_optim_theta_arr(arr, theta_arr, res_par=-2):
    # minimizes the size of the input theta array while preserving the local maxima and minima of the eikcoefs.
    ...
    #comb_peaks = np.sort(np.abs(np.concatenate((peaks-3*diff, peaks-2*diff, peaks, peaks+2*diff, peaks+3*diff, peaks2-3*diff, peaks2, peaks2+3*diff, np.array([0, len(theta_arr)-1-diff])))))
    ...
    if len(all_peaks) == 0:
        ...
        for i in range(num_arrays):
            if i == 0:
                 ...
                 theta_new_arr1 =np.concatenate((theta_new_arr, theta_arr[relevant_peaks], np.interp(split_b_arr[i], split_b_arr[i+1], split_theta_arr[i+1]))) #array containing the theta points around the extremum
            elif i == num_arrays-1:
        ...
        if mode == 1:
            ...
            #theta_new_arr1 = equalize(np.sort(np.unique(theta_new_arr1)),all_peaks, len(theta_new_arr)-len(theta_arr))
            theta_new_arr1 = equalize(np.sort(np.unique(theta_new_arr1)), np.concatenate((split_b_arr[0],b_arr[relevant_peaks], split_b_arr[0][::-1])), relevant_peaks, len(theta_arr)-len(theta_new_arr2), spacing)
