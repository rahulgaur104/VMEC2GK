"""
Takes an axisymmetric, up-down symmetric VMEC equilibrium and calculates the geometric
coefficients required for a gyrokinetics run (with GX/GS2).
"""
...
def vmec_to_bishop(
    ...
    """
    Convert a VMEC equilibrium file to bishop data.

    Parameters
    ----------
    vmec_filename: Path
        Relative/absolute path to a VMEC equilibrium file, in netCDF4 format.
    surf_idx: int
        Index of the flux surface to analyse.
    norm_scheme: int
        If 1, a_N is effective minor radius of the LCFS such that pi*a_N**2 = area
        enclosed by the LCFS, so B_N = Phi_LCFS/(pi*a_N**2). If 2, a_N is the minor
        radius of the LCFS, so B_N = F/a_N, where F is poloidal current on the local
        equilibrium surface.
    nperiod: int
        The number of times the flux tube goes around is 2*nperiod-1.
    fac: int, default is 2
        The ratio between the number of poloidal points in real space and the number of
        modes in Fourier space, divided by 2.
    """

    # Get some info from the filename
    # TODO use regex, match "pres_scale_\d+"
    try:
        pres_scale = int(vmec_filename.stem.split("_")[-1])
    except ValueError:
        pres_scale = 0

    # get the type of eqbm, i.e., Miller and/or negtri/postri
    # TODO use regex, match exactly one of 'eqbm', 'postri', 'negtri', etc.
    try:
        eqbm_type = vmec_filename.name.split("_")[2]
    except IndexError:
        eqbm_type = "eqbm"

    rtg = Dataset(vmec_filename, "r")
    ...
    if totl_surfs < surf_max:
        surf_min = totl_surfs - 7
        surf_max = totl_surfs - 1
        surf_idx = totl_surfs - 4
        warn_msg = dedent(
            f"""\
            Total number of surfaces > maximum surface index.
            Setting surf_max={surf_max}, surf_min={surf_min}, surf_idx={surf_idx}.
            """
        )
    ...
    psi_half = (
        rtg.variables["chi"][surf_min + 1 : surf_max + 1]
        + 0.5 / (totl_surfs - 1) * dpsids
    )
    ...
    # Fourier coeffs of R. Full mesh quantity.
    ...
    # Fourier coeffs of the Jacobian
    gmnc = rtg.variables["gmnc"][surf_min + 1 : surf_max + 1].data
    g_jac = ifft_routine(gmnc, xm_nyq, "e", fixdlen, fac)

    # half mesh quantity
    lmns = rtg.variables["lmns"][surf_min + 1 : surf_max + 1].data
    lmns = ifft_routine(lmns, xm, "o", fixdlen, fac)

    # half mesh quantity
    B_sub_zeta = rtg.variables["bsubvmnc"][surf_min + 1 : surf_max + 1].data
    B_sub_zeta = ifft_routine(B_sub_zeta, xm_nyq, "e", fixdlen, fac)
    # half-mesh quantity
    B_sub_theta = rtg.variables["bsubumnc"][surf_min + 1 : surf_max + 1].data
    ...
    # mode being 0 (default) or 1, and the theta_st expression containing a minus or
    # plus. Confirm this is the case and refactor.
    if R[0][0] < R[0][idx0]:
        ...
        B_theta_vmec = np.sqrt(
            np.abs(
                B_sub_theta
                * Phi_LCFS
                / (2 * np.pi * np.reshape(q_vmec_half, (-1, 1)) * g_jac)
            )
        )
    ...
    rho = np.array(
        [np.sqrt(np.abs(Phi_half[i] / Phi_LCFS)) for i in range(no_of_surfs)]
    )
    ...
    F_half_2 = np.abs(
        np.array(
            [
                1
                / ctrap(2 * g_jac[i] / (Phi_LCFS * R[i] ** 2), theta_vmec, initial=0)[
                    -1
                ]
                for i in range(no_of_surfs)
            ]
    ...
    print(
        "Note that theta_st is not uniformly spaced so derm can't be central "
        "difference.\n Using 2nd order non-uniform FD on a non-uniform grid"
    )

    for i in range(0, no_of_surfs):
        # Interpolation of full grid quantities to a half-grid surfaces requires
        # i - > i-1.
    ...
    for i in range(no_of_surfs):
        ...
        phi_n[i, :] = np.concatenate(
            (phi[phi >= 0] - np.pi / 2, phi[phi < 0] + 3 * np.pi / 2)
        )

    u_ML = np.arctan2(derm(Z, "l", "o"), derm(R, "l", "e"))

    dl = np.sqrt(derm(R, "l", "e") ** 2 + derm(Z, "l", "o") ** 2)
    # The way u is defined in the notes, it should decrease with increasing l which is
    # different from the value being calculated here
    ...
    B0 = [
        np.sqrt(B_p_t_cart[i] ** 2 + (F[i] / R[i, :]) ** 2)
        for i in np.arange(surf_min, no_of_surfs)
    ]
    ...
    for i in np.arange(no_of_surfs):
        ...
        q_int[i] = 2 * np.trapz(
            F[i]
            / (2 * R[i] * np.pi * np.abs(np.sqrt(dpsidR[i] ** 2 + dpsidZ[i] ** 2))),
            L[i],
        )
        jac_chk = F[i] * jac / (R[i] * q_int[i])
        # print(
        #     (np.max(jac_chk[i])-np.min(jac_chk[i]))/np.mean(np.abs(jac_chk[i])),
        #     sep = '   ',
        # )
    ...
    shat = (
        rho[rel_surf_idx]
        / q_vmec_half[rel_surf_idx]
        * q_spl.derivative()(psi[rel_surf_idx])
        / rho_spl.derivative()(psi[rel_surf_idx])
    )
    ...
    L_st_ex = np.concatenate(
        (np.array([0.0]), np.cumsum(np.sqrt(np.diff(R_ex) ** 2 + np.diff(Z_ex) ** 2)))
    )

    # the factor of 2 in the front cancels with the the 2 in 2*pi in the expression for
    # shat_test
    a_s = -(
        2 * q_vmec_half[rel_surf_idx] / F[rel_surf_idx] * theta_st_com_ex
        + 2
        * F[rel_surf_idx]
        * q_vmec_half[rel_surf_idx]
        * ctrap(1 / (R_ex**2 * B_p_ex**2), theta_st_com_ex, initial=0)
    )
    b_s = -(
        2
        * q_vmec_half[rel_surf_idx]
        * ctrap(1 / (B_p_ex**2), theta_st_com_ex, initial=0)
    )
    c_s = (
        2
        * q_vmec_half[rel_surf_idx]
        * ctrap(
            (2 * np.sin(u_ML_ex) / R_ex - 2 / R_c_ex) * 1 / (R_ex * B_p_ex),
            theta_st_com_ex,
            initial=0,
        )
    )
    ...
    if norm_scheme == 1:
        ...
        a_N = rtg.variables["Aminor_p"][
            :
        ].data  # imported from VMEC, same as the a_N 3 lines above
    ...
    shat_test = (
        -(1 / (2 * np.pi * (2 * nperiod - 1) * q_vmec_half[rel_surf_idx]))
        * (rho[rel_surf_idx] * dpsidrho)
        * (a_s[-1] * dFdpsi + b_s[-1] * dPdpsi - c_s[-1])
    )
    ...
    # The 0.5 in the front is to cancel the factor of 2 in that has been used in a_s
    # b_s and c_s
    # aprime_bish = R[rel_surf_idx]*B_p[rel_surf_idx]*(
    #     a_s*dFdpsi_4_ball +b_s*dPdpsi - c_s
    # )*0.5
    ...
    dtdr_st_ex = -(dt_dR_ex * dpsidR_ex + dt_dZ_ex * dpsidZ_ex) / np.sqrt(
        dpsidR_ex**2 + dpsidZ_ex**2
    )
    ...
    aprime = -(
        np.reshape(q_vmec_half, (-1, 1)) * dtdr_st_ex + dqdr_ex * theta_st_com_ex
    )
    ...
    # always use this method of calculating dt_st_l_ex instead of
    # derm(theta_st_com_ex, 'l','o'), future me
    ...
    dtdr_st_bish = -(aprime_bish_1 + dqdr_ex * theta_st_com_ex) / np.reshape(
        q_vmec_half, (-1, 1)
    )
    ...
    gds2 = (
        ...
        * (
            1 / R_ex**2
            + (dqdr_ex * theta_st_com_ex) ** 2
            + (np.reshape(q_vmec_half, (-1, 1))) ** 2
            * (dtdr_st_ex**2 + (dt_st_l_ex / dl_ex) ** 2)
            + 2
            * np.reshape(q_vmec_half, (-1, 1))
            * dqdr_ex
            * theta_st_com_ex
            * dtdr_st_ex
        )
    ...
    # dB2l       = derm(B2, 'l', 'e')
    # dB2l_dl_ex = nperiod_data_extend(
    #     dB2l[rel_surf_idx], nperiod, istheta=0, par='o'
    # )/dl_ex
    dBl = derm(B, "l", "e")
    ...
    if want_2_filter == 1:
        print(
            "filter-L2-norm-err = %.4f\n"
            % (
                np.linalg.norm(
                    dBl[2] - np.concatenate((np.array([0]), sf(dBl[2], 9, 2)[1:]))
                )
            )
        )
    ...
    dB_dr = (
        derm(B_ex, "l", "e") / dt_st_l_ex * dtdr_st_ex
        + nperiod_data_extend(derm(B, "r")[rel_surf_idx], nperiod)
        / psi_diff[rel_surf_idx]
        * dpsi_dr_ex
    )
    ...
    #     + aprime_bish_1*F/R_ex*sf(dB2l_ex/dl_ex, 9, 2)*1/B_ex
    # )
    gbdrift_bish = (
        dpsidrho
        / (B_ex**3)
        * (
            -2 * B_ex**2 * dBdr_bish / dpsi_dr_ex
            + aprime_bish_1 * F / R_ex * dB2l_dl_ex * 1 / B_ex
        )
    )
    # gbdrift = dpsidrho*(
    #     -2/B_ex*dBdr_bish/dpsi_dr_ex + 2*aprime*F/R_ex*1/B_ex**3*dBl_ex/dl_ex
    # )
    gbdrift = dpsidrho * (
        -2 / B_ex * dBdr_bish / dpsi_dr_ex
        + 2 * aprime * F / R_ex * 1 / B_ex**3 * dBl_dl_ex
    )
    gbdrift0 = (
        1
        * 2
    ...
    spl_st_to_eqarc_theta = cubspl(
        theta_st_com_ex[theta_st_com_ex <= np.pi], theta_eqarc
    )
    ...
    bishop_dict = {
        ...
        "B_N": B_N,
        "mag_well": mag_well,
        "mag_local_peak": mag_local_peak,
        "B_local_peak": int(B_local_max_0_idx),
        "pres_scale": pres_scale,
        "eqbm_type": eqbm_type,
        "surf_idx": surf_idx,
        "high_res_fac": 42,
        "qfac": q_vmec_half[rel_surf_idx],
        "shat": shat,
        "dqdpsi": dqdpsi,
        "P": P[rel_surf_idx],
        "dPdpsi": dPdpsi,
        "F": F[rel_surf_idx],
        "dFdpsi": dFdpsi,
        "rho": rho[rel_surf_idx],
        "dpsidrho": dpsidrho,
        "theta_st": theta_st_com_ex,
        "nperiod": nperiod,
        "a_s": a_s,
        "b_s": b_s,
        "c_s": c_s,
        ...
        "B_ex": B_ex,
        "dBl_ex": dBl_ex,
        "dt_st_l_ex": dt_st_l_ex,
        "dtdr_st_ex": dtdr_st_ex,
        "dl_ex": dl_ex,
        "u_ML_ex": u_ML_ex,
        "gds2_ex": gds2,
        "spl_st_to_geo_theta": spl_st_to_geo_theta,
