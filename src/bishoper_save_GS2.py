#!/usr/bin/env python3
"""
This script creates and saves a grid.out file for a GS2 run using local equilibrium data from a dictionary. There is also a provision to change shat and dPdpsi locally. 
...
def bishop_save(shat_n, dPdpsi_n, pfac):
        ...
        gradpar_n = np.abs(a_N/(B_ex)*(-B_p_ex)*(dt_st_l_ex/dl_ex)) # gradpar is b.grad(theta)
        ...
        gbdrift_n = dpsidrho*(-2/B_ex*dBdr_bish_n/dpsi_dr_ex + 2*aprime_n*F/R_ex*1/B_ex**3*dBl_ex/dl_ex)
        ...
        Rprime_ex = nperiod_data_extend(np.sin(u_ML_ex[theta_st_com_ex <= np.pi]), nperiod, istheta=0, par='e')
        Zprime_ex = -nperiod_data_extend(np.cos(u_ML_ex[theta_st_com_ex <= np.pi]), nperiod, istheta=0, par='o')


        if nperiod == 1:
            theta_st_com_ex_uniq = theta_st_com_ex
        else:
            theta_st_com_ex_uniq = find_optim_theta_arr(np.vstack((gradpar_n, cvdrift_n, gbdrift_n, gbdrift0_n, gds2_n, gds21_n, gds22_n, B_ex)), theta_st_com_ex)

        if theta_st_com_ex_uniq[0] != 0:
            print('temp fix at 270 bishoper_save')
            theta_st_com_ex_uniq[0] = 0.

        # The next 12 lines make sure that the larger theta array is chosen 
        # between [0, np.pi] and [np.pi, 2*np.pi]. Doing so and then extending to nperiod>1 will make the B symmetrix about the global extrema
        ...
        # if there is a magnetic well symmetrize provides a theta grid(temp1) corresponding to symmetric B values which can replace the theta grid in the well from theta_st_com_uniq_sym
        override1 = 0
        if mag_well == "True" and override1 == 0:
            override0 = 0
            if override0 == 0:
                #temp1, _ =  symmetrize(theta_st_com[B_local_max_0_idx:], np.interp(theta_st_com[B_local_max_0_idx:], theta_st_com_ex, B_ex), mode=2, spacing=3)
                temp1, _ =  symmetrize(theta_st_com[B_local_max_0_idx:], np.interp(theta_st_com[B_local_max_0_idx:], theta_st_com_ex, B_ex), mode=2, spacing=4)
                theta_st_com_uniq_sym = np.unique(np.concatenate((theta_st_com_uniq_sym[:B_local_max_0_idx-1], temp1, theta_st_com_uniq_sym[theta_st_com_uniq_sym > temp1[-1]])))
            else:
                temp1, _ =  symmetrize(theta_st_com[:], np.interp(theta_st_com[:], theta_st_com_ex, B_ex), mode=2, spacing=3)
                theta_st_com_uniq_sym = np.unique(np.concatenate((theta_st_com_uniq_sym[:B_local_max_0_idx-1], temp1, theta_st_com_uniq_sym[theta_st_com_uniq_sym > temp1[-1]])))
        ...
        for i in range(ntheta+1):
            if i == 0:
                data[0, :5] = np.array([int((ntheta-1)/2), 0., shat, 1.0 , qfac]) 
            else:
                ...
                A2.append('    %.9f    %.9f    %.9f    %.9f\n'%(gbdrift_ball[i-1], gradpar_ball[i-1], grho_ball[i-1], theta_ball[i-1]))
                ...
                A8.append('    %.9f    %.9f    %.9f\n'%(-qfac*reflect_n_append(theta_st_com_ex_uniq_sym, 'o')[i-1], aprime_ball[i-1], theta_ball[i-1]))
        ...
        if mag_well == 'True':
            ...
            nlambda = len(lambda_create(B_ex_uniq[theta_st_com_ex_uniq_sym <= np.pi][temp1.item():]))
            lambda_arr = lambda_create(B_ex_uniq[theta_st_com_ex_uniq_sym <= np.pi][temp1.item():]/B_N)
        ...
        if isinstance(pfac, int) != 1:
            ...
            fname_in_txt_rescaled = '{0}/{1}/{2}/{3}_{4}_{5}'.format(parnt_dir_nam, 'output_files','GS2_grid_files', char, 'eikcoefs', name_suffix)
        else:
            fname_in_txt_rescaled = '{0}/{1}/{2}/{3}_{4}_{5}_{6}'.format(parnt_dir_nam, 'output_files','GS2_grid_files', char, 'eikcoefs', pfac, 'dPdpsi')
