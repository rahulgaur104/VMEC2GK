#!/usr/bin/env python3
"""
This script takes an axisymmetric, up-down symmetric VMEC equilibrium and calculates
the geometric coefficients required for a gyrokinetics run (with GX/GS2).
"""
import os
import re
import pickle
...
R = ifft_routine(vs.rmnc_coeffs.T, xm, "e", fixdlen, fac)
print("rmnc_error = ", np.max(np.abs(rmnc[2]-vs.rmnc_coeffs.T[0])))
...
print(np.max(np.abs(bmnc[2]-vs.bmnc_coeffs.T[0])))
#B = ifft_routine(bmnc, xm_nyq, "e", fixdlen, fac)
B = ifft_routine(vs.bmnc_coeffs.T, xm_nyq, "e", fixdlen, fac)

gmnc = rtg.variables["gmnc"][
    surf_min + 1 : surf_max + 1
].data  # Fourier coeffs of the Jacobian

print(np.max(np.abs(gmnc[2]-vs.gmnc_coeffs.T[0])))
#g_jac = ifft_routine(gmnc, xm_nyq, "e", fixdlen, fac)
g_jac = ifft_routine(vs.gmnc_coeffs.T, xm_nyq, "e", fixdlen, fac)
...
B_sub_zeta = rtg.variables["bsubvmnc"][
    surf_min + 1 : surf_max + 1
].data  # half mesh quantity
print(np.max(np.abs(B_sub_zeta[2]-vs.bsubvmnc_coeffs.T[0])))

#B_sub_zeta = ifft_routine(B_sub_zeta, xm_nyq, "e", fixdlen, fac)
B_sub_zeta = ifft_routine(vs.bsubvmnc_coeffs.T, xm_nyq, "e", fixdlen, fac)

B_sub_theta = rtg.variables["bsubumnc"][
    surf_min + 1 : surf_max + 1
].data  # half-mesh quantity
print(np.max(np.abs(B_sub_theta[2]-vs.bsubumnc_coeffs.T[0])))
#B_sub_theta = ifft_routine(B_sub_theta, xm_nyq, "e", fixdlen, fac)
B_sub_theta = ifft_routine(vs.bsubumnc_coeffs.T, xm_nyq, "e", fixdlen, fac)
...
# RG: The above line is correct. Choosing 0 or 1 also changes the definition of theta_vmec
# in terms of theta_pest.
if R[0][0] < R[0][idx0]:
...
F_half_2 = np.abs(
    np.array(
        [
            1 / ctrap(2 * g_jac[i] / (Phi_LCFS * R[i] ** 2), theta_vmec, initial=0)[-1]
            for i in range(no_of_surfs)
        ]
    )
)
...
for i in np.arange(no_of_surfs):
    ...
    q_int[i] = 2 * np.trapz(
        F[i] / (2 * R[i] * np.pi * np.abs(np.sqrt(dpsidR[i] ** 2 + dpsidZ[i] ** 2))),
        L[i],
    )
...
b_s = -(
    2 * q_vmec_half[rel_surf_idx] * ctrap(1 / (B_p_ex**2), theta_st_com_ex, initial=0)
)
c_s = (
    2
    * q_vmec_half[rel_surf_idx]
...
# The 0.5 in the front is to cancel the factor of 2 in that has been used in a_s, b_s
# and c_s
...
gds2 = (
    ...
    * (
        ...
        * (dtdr_st_ex**2 + (dt_st_l_ex / dl_ex) ** 2)
        + 2 * np.reshape(q_vmec_half, (-1, 1)) * dqdr_ex * theta_st_com_ex * dtdr_st_ex
    )
    * 1
    / (a_N * B_N) ** 2
)
